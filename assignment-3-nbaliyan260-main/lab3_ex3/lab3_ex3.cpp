...
// Lab 3, Exercise 3: Batched Multicast for Improved Throughput
// Multicasts tiles_per_batch (10) tiles per semaphore handshake to reduce overhead.
// Sender core also participates in compute (from Exercise 2).
...
void multicast_tensor_tensix(
    ...
    const uint32_t num_total_cores = num_receivers + 1;
    ...
    TT_FATAL(n_tiles % TILES_PER_BATCH == 0,
        "Total number of tiles ({}) must be divisible by tiles_per_batch ({})", n_tiles, TILES_PER_BATCH);
    ...
    // CB: double-buffered batches -> 2 * TILES_PER_BATCH tiles per CB side
    constexpr uint32_t cb_tiles = 2 * TILES_PER_BATCH;
    create_cb(prog_state.program, all_cores_logical, cb_tiles, tt::CBIndex::c_0);
    create_cb(prog_state.program, all_cores_logical, 2, tt::CBIndex::c_16);

    // Sender kernel compile args: TensorAccessorArgs + tiles_per_batch
    std::vector<uint32_t> mcast_sender_compile_args = {TILES_PER_BATCH};
    ...
    KernelHandle mcast_sender_id = CreateKernel(
        prog_state.program,
        OVERRIDE_KERNEL_PREFIX "ttnn/examples/lab3_ex3/kernels/dataflow/mcast_sender.cpp",
        sender_core_logical,
        mcast_sender_config);

    // Receiver kernel compile args: tiles_per_batch
    ...
    KernelHandle mcast_receiver_id = CreateKernel(
        prog_state.program,
        OVERRIDE_KERNEL_PREFIX "ttnn/examples/lab3_ex3/kernels/dataflow/mcast_receiver.cpp",
        receiver_cores_logical,
        mcast_receiver_config);

    // Writer kernel on ALL cores
    ...
    KernelHandle write_tiles_id = CreateKernel(
        prog_state.program,
        OVERRIDE_KERNEL_PREFIX "ttnn/examples/lab3_ex3/kernels/dataflow/write_tiles.cpp",
        all_cores_logical,
        write_tiles_config);

    // Compute kernel on ALL cores
    ...
    log_info(tt::LogAlways,
        "Launching batched multicast ({} tiles/batch) of {} tiles: sender + {} receivers",
        TILES_PER_BATCH, n_tiles, num_receivers);
    ...
    log_info(tt::LogAlways, "Batched multicast complete");
...
int main() {
    ...
    try {
        ...
        static_assert(n_tiles % TILES_PER_BATCH == 0,
            "Total tiles must be divisible by TILES_PER_BATCH");

        log_info(
            tt::LogAlways,
            "Lab3 Ex3: {}x{} tensor ({} tiles), {} tiles/batch, sender + {} receivers",
            M, N, n_tiles, TILES_PER_BATCH, num_receivers);
